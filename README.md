...
- **サービス構成**: [`docker/Dockerfile`](docker/Dockerfile) は共通の土台（`runner-base`）から `runner`（Web = `next start`）と `worker`（Cloud Run Jobs 用。`ENTRYPOINT` で `tsx src/worker/index.ts` を起動）にステージが分かれている。本番で常時稼働する Cloud Run サービスは `runner` のみで、`worker` は CSV エクスポート等の依頼のたびに Cloud Run Jobs として単発起動する設計（構築状況は [`docs/todo/TODO.md`](docs/todo/TODO.md) を参照）。
