...
| タスクのタイムアウト | `600`秒（既定のまま。「分」単位の画面では `10`分） | 使う機能の想定処理時間に応じて調整する |
...
> **現在、この節が設定する環境変数を読むコードはありません。** マスタCSVダウンロードが同期方式に作り直され、appからworkerを呼び出す仕組み（`WORKER_INVOKE_MODE` 等）を使わなくなったためです（設計書 [`30_CSVダウンロード.md`](../02_basic-design/master/30_CSVダウンロード.md) §30.1.9）。**「マスタ情報Excel取得」の実装時（設計書 [`40_マスタ情報Excel取得.md`](../02_basic-design/master/40_マスタ情報Excel取得.md) §40.7.4）にこの環境変数を使うようになります。** 実装時は `src/shared/config/env.ts` に対応する環境変数を改めて追加する必要があります。
