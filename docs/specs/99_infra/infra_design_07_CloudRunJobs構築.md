...
手順6（該当する場合のみ）。**worker を使う機能を有効にする場合だけ**実施する手順。worker 用イメージの用意・Cloud Run Jobs の作成・環境変数の設定。
...
**worker はジョブワーカー（pg-boss）を使う機能があるときだけ必要です。** マスタCSVダウンロードは、appのリクエスト内で完結する同期方式のためworkerを使いません（設計書 [`30_CSVダウンロード.md`](../02_basic-design/master/30_CSVダウンロード.md) §30.1.9）。現時点では、「マスタ情報Excel取得」（設計書 [`40_マスタ情報Excel取得.md`](../02_basic-design/master/40_マスタ情報Excel取得.md)）がworkerを使う機能です。案件によって worker を使う機能を採用しない場合、本書（07.1〜08.1）は不要です。

worker の実行方式（Cloud Run Jobs として単発実行し、app が起動する）の一般的な作り方は本書の以降の節で説明する。個々の機能ごとの設計は、その機能の設計書を参照すること。

## 07.1.2 worker 用のコンテナイメージを用意する

**ここで行うこと**: worker（時間のかかる処理を裏側で動かすプログラム）を、Google Cloud 上に送信して登録します。次の4ステップです。
...
| メモリ | `512 MiB`（既定） | app と同じ想定で十分。使う機能の処理内容に応じて調整する |

続いて、画面下方（またはコンテナ編集の外）にある以下の項目を確認します（タイムアウト以外は既定値のままで問題ありません）。

| 項目 | 設定値 | 理由 |
| --- | --- | --- |
| タスクの数 | `1`（既定） | pg-boss のキュー全体を1タスクで処理する設計 |
| 並列実行数（並列処理） | `1`、または「できる限り多くのタスクを同時実行する」（既定） | タスクの数が1なのでどちらでも結果は同じ |
| タスクごとの最大再試行回数 | `3`（既定のまま） | 再試行しても安全なように、処理は冪等（同じ内容を何度実行しても結果が変わらない）に作る |
| タスクのタイムアウト | `900`秒（15分）。既定の `600` 秒から変更する（「分」単位の画面では `15`分） | 「マスタ情報Excel取得」は Cloud Run Jobs の起動待ち（1〜3分）を含み、起動待ちの上振れや処理量が増えた場合の余裕として 900 秒にしている（設計書 [`40_マスタ情報Excel取得.md`](../02_basic-design/master/40_マスタ情報Excel取得.md) §40.7.3）。他の機能を載せる場合はその想定処理時間に応じて調整する |
| **サービス アカウント** | ひとまず既定のまま | 専用サービスアカウントを作成後、そちらへ差し替える |
...
> `DATABASE_URL` が未設定の場合、worker はコンテナ起動直後に「環境変数の検証に失敗しました」を出して異常終了します（[`src/shared/config/env.ts`](../../../src/shared/config/env.ts)）。**他の4項目は未設定でも起動はしますが、ファイルの生成や保存を行う処理が失敗します。**

> **worker 用のサービスアカウントにも参照権限が必要です。** [05.1.5 の事前準備2](infra_design_05_CloudRun構築.md#0515-環境変数を設定する) で app の実行サービスアカウントへ付与済みですが、worker が別のサービスアカウント（専用アカウントへ差し替えた場合など）を使う場合は、`database-url` / `supabase-service-role-key` それぞれの「権限」タブで、そのアカウントにも `roles/secretmanager.secretAccessor` を追加してください。

## 07.1.5 app 側に worker の起動先を設定する

> **この節の環境変数は「マスタ情報Excel取得」（設計書 [`40_マスタ情報Excel取得.md`](../02_basic-design/master/40_マスタ情報Excel取得.md) §40.7）が実際に使います。** app は依頼を受け付けるたびに、[`src/shared/jobs/invoke-worker.ts`](../../../src/shared/jobs/invoke-worker.ts) からこの設定を読んで Cloud Run Jobs を起動します。`WORKER_INVOKE_MODE=cloud-run-job` を設定した場合、残りの3項目が未設定だと app はコンテナ起動直後に「環境変数の検証に失敗しました」を出して異常終了します（[`src/shared/config/env.ts`](../../../src/shared/config/env.ts)）。
...
> **この付与により、[05.1.4](infra_design_05_CloudRun構築.md#0514-サービスの設定値) の「権限を持たない専用サービスアカウントへの差し替え」は「必要な権限だけを持つ専用サービスアカウントへの差し替え」に変わります。** 権限ゼロは成立しなくなりますが、Cloud Run Jobs 1個の実行権限のみであり、既定のサービスアカウントが持つ編集者権限とは比較にならないほど狭い権限です。**この権限は「マスタ情報Excel取得」で実際に使われます。** app が依頼のたびに Cloud Run Jobs を起動するため（§07.1.5）、この権限がないと起動要求が拒否されます。ただし起動要求が失敗しても依頼自体は失敗せず、警告としてログに残るだけで、依頼は順番待ちの列に残ります（設計書 [`40_マスタ情報Excel取得.md`](../02_basic-design/master/40_マスタ情報Excel取得.md) §40.10）。**権限の付け忘れは画面上ではエラーにならず、「いつまでも実行中のまま」という形で現れます。**
