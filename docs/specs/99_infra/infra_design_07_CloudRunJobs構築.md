...
| タスクのタイムアウト | `900`秒（15分）。既定の `600` 秒から変更する（「分」単位の画面では `15`分） | 「マスタ情報Excel取得」は Cloud Run Jobs の起動待ち（1〜3分）と意図的な待ち時間（約2分）を含むため、既定の600秒では足りない（設計書 [`40_マスタ情報Excel取得.md`](../02_basic-design/master/40_マスタ情報Excel取得.md) §40.7.3）。他の機能を載せる場合はその想定処理時間に応じて調整する |
