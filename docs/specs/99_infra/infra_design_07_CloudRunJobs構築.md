...
**worker はジョブワーカー（pg-boss）を使う機能があるときだけ必要です。** マスタCSVダウンロードは、appのリクエスト内で完結する同期方式のためworkerを使いません（設計書 [`30_CSVダウンロード.md`](../02_basic-design/master/30_CSVダウンロード.md) §30.1.9）。現時点では、[`docs/todo/TODO.md`](../../todo/TODO.md) の「マスタ情報Excel取得機能（workerサンプル）」がworkerを使う想定の機能です。案件によって worker を使う機能を採用しない場合、本書（07.1〜08.1）は不要です。
...
| **サービス アカウント** | ひとまず既定のまま | [`docs/todo/TODO.md`](../../todo/TODO.md) の「Cloud Run Jobs本番構成の構築」6 で専用サービスアカウントへ差し替える |
...
> **worker 用のサービスアカウントにも参照権限が必要です。** [05.1.5 の事前準備2](infra_design_05_CloudRun構築.md#0515-環境変数を設定する) で app の実行サービスアカウントへ付与済みですが、worker が別のサービスアカウントを使う場合（[`docs/todo/TODO.md`](../../todo/TODO.md) の「Cloud Run Jobs本番構成の構築」6 で専用アカウントへ差し替えた場合など）は、`database-url` / `supabase-service-role-key` それぞれの「権限」タブで、そのアカウントにも `roles/secretmanager.secretAccessor` を追加してください。

## 07.1.5 app 側に worker の起動先を設定する

> **現在、この節が設定する環境変数を読むコードはありません。** マスタCSVダウンロードが同期方式に作り直され、appからworkerを呼び出す仕組み（`WORKER_INVOKE_MODE` 等）を使わなくなったためです（設計書 [`30_CSVダウンロード.md`](../02_basic-design/master/30_CSVダウンロード.md) §30.1.9）。将来、appからの依頼に応じてworkerをその場で起動したい機能ができたときのために手順を残していますが、実装時は `src/shared/config/env.ts` に対応する環境変数を改めて追加する必要があります。
