...
> 画面が本書と異なる場合は [1.6](infra_design_00_概要と全体構成.md#16-uiは資料と異なる可能性あり) を参照してください。
...
いずれの場合も、問題のある内容を反映してしまったときは元の状態へ戻せます（[11.5](infra_design_07_構築後の運用.md#115-以前の状態へ戻す切り戻し)）。

> **ドキュメント（`.md` / `docs/` 配下）のみの変更は、`main` へ直接コミットしてよい**という例外を設けています。アプリの動作に影響しないためです。詳細は [`git.md`](../../development/git.md#ドキュメントだけの変更でciを実行しないmainへ直接push) を参照してください。

手順は [`git.md`](../../development/git.md) にあります。
