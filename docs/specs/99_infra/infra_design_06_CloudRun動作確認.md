...
| 確認内容                   | 期待する結果       | 失敗した場合                                |
| ---------------------- | ------------ | ------------------------------------- |
| `/api/health`          | `status: ok` | アプリが起動していません → [12.1 章](infra_design_12_トラブルシュート.md#121-トラブルシュート)   |
...
ブラウザで `$base` を開き、[04.1.5](infra_design_04_本番データベース初期化.md#0415-初期管理者を登録する) で登録した管理者でログインして動作確認をします。
