...
| 2026-08-24 | [`pnpm prisma:migrate -- --name ...` の書き方が引数を渡しきれずハングすることがある](prisma-cli.md#pnpm-prismamigrate----name--の書き方が引数を渡しきれずハングすることがある) | `pnpm exec prisma migrate dev --create-only --name ...` で回避。放置するとDBアドバイザリロックを握ったプロセスが残る |
