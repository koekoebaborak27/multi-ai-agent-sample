...
| 2026-08-22 | [パスワード再発行機能を本番へ反映する](#2026-08-22-パスワード再発行機能を本番へ反映する) | 追加系マイグレーションの適用順・Secret Manager権限の付け忘れ・環境変数の入力ミス |
...
## 2026-08-22 パスワード再発行機能を本番へ反映する

**2026-08-22 に実施・完了した。** 手順そのものは [`infra_design_09_メール送信.md` §09.1.7](../../specs/99_infra/infra_design_09_メール送信.md#0917-本番cloud-runに設定する)（今回の作業で加筆済み）のとおり。ここには実測値と詰まった点のみ残す。

**適用順序: 先にDBマイグレーション、その後PRマージ。** `User.email` は `String? @unique`（NULL許容）、`PasswordResetToken`・`EmailChangeToken` は新規テーブルのみで、既存コードは一切参照しないため、マイグレーション適用は既存機能に無影響（[`01_データベース.md`](../../specs/02_basic-design/password-reset/01_データベース.md#012-user-の変更)）。この前提のもと、マイグレーション適用 → PRマージ・自動デプロイの順にした。逆順（マージ→マイグレーション）だと、新コードが公開された直後からデプロイ完了までの数分間、ログイン画面の「パスワードを忘れた場合はこちら」リンクをクリックすると表が無くDBエラーになる状態が発生する。

**PRマージ前にCIが落ちていた。** `verify`(`format:check`) が e2e テスト5ファイルのPrettierフォーマット崩れで失敗。`pnpm format` で機械的に直るコード内容に影響しない差分だったため、その場で修正してpushした。

**Secret Managerの権限は自動では付かない。** `smtp-password` を作成した直後の「権限」タブには、プロジェクト全体から継承された「編集者」等の行しか無く、Cloud Runの実行アカウント（`contract-app` は `<プロジェクト番号>-compute@developer.gserviceaccount.com`）への「Secret Manager のシークレット アクセサー」は**明示的に追加しないと付かない**。継承された「編集者」ロールにはSecret Managerの値を読む権限が含まれないため、この付与を飛ばすと `SMTP_PASSWORD` の参照が失敗する。実行アカウント名を忘れている場合は、Cloud Runのサービス詳細 →「新しいリビジョンの編集とデプロイ」→「セキュリティ」タブで確認できる（デプロイしなければ設定は変わらない）。

**`MAIL_FROM` / `SMTP_USER` に個人のGmailアドレスを入力しかけた。** Cloud Runの環境変数入力時、送信専用に作った専用アカウント（例: `xxxxx@gmail.com`）ではなく、ログイン中の個人アカウントのアドレスを一瞬入力してしまった。デプロイ前のスクリーンショット確認で気づき、修正してから反映した。**この2項目は必ず「送信専用の新規アカウント」のアドレスであることを、デプロイ前にスクリーンショットで確認する。**

**確認結果。** マイグレーション適用（`20260821023637_add_password_reset`）→ PR #27マージ・自動デプロイ→ 本番ログイン画面に「パスワードを忘れた場合はこちら」表示を確認 → 本番で実際にパスワード再発行を申し込み、メールが届くことを確認、で完了。
