...
**次のセッションでは、[お知らせ管理機能](#お知らせ管理機能) の実装工程4（トップ画面の表示。`NewsFeed`・`NewsItem`）に着手する**（工程3のモジュールのリネームと土台実装は2026-08-24に完了済み。→ [履歴](history/2026-08-w4.md#2026-08-24-お知らせ管理機能のモジュールリネームと土台実装工程3を完了)）。[公開用テンプレートの切り出し](#公開用テンプレートの切り出し)（2026-08-23 に方針を確定済み。→ [履歴](history/2026-08-w3.md#2026-08-23-公開用テンプレートの切り出し方針を確定)）は、この後に着手する。
...
- [ ] 4. トップ画面の表示を実装する（`NewsFeed`・`NewsItem`、最新20件取得、カテゴリ色分け。→ [設計書§10](../specs/02_basic-design/news/10_トップ画面お知らせ表示.md)）
