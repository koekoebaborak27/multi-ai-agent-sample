...
| **マスタ分類の見直し** | **4 / 4** |
| **契約先・契約の管理画面整備** | **1 / 5（要件定義完了、設計未着手）** |
...
**契約先・契約の管理画面整備の要件定義が完了した**（2026-08-19）。契約先・契約は一覧画面しかなく詳細・編集・削除の画面が無いという指摘を受け、マスタ管理画面と同じ構成（一覧検索→詳細→編集/新規登録は別ページ→削除）に揃える方針を確定した。詳細は [履歴](history/2026-08-w3.md#2026-08-19-契約先契約の管理画面整備の要件定義を実施) を参照。

**次にやることは、契約先・契約の管理画面整備の設計書（`docs/specs/`配下）を作成すること。** マスタ機能の設計書（[`docs/specs/02_basic-design/master/`](../specs/02_basic-design/master/README.md)）を土台に、画面項目・データモデル（変更点があれば）・確認画面の要否などを決める。
...
## 契約先・契約の管理画面整備
マスタ機能の進め方（設計6項目→製造工程をチェックリスト化→工程を製造→単体テスト）に倣う。3〜5番の具体的な工程数は、2番の設計書が固まってから確定させる。

- [x] **1. 要件定義を行う**（2026-08-19）→ [履歴](history/2026-08-w3.md#2026-08-19-契約先契約の管理画面整備の要件定義を実施)
- [ ] **2. 設計書を作成する**（`docs/specs/`配下。マスタ機能の設計書が土台）
- [ ] **3. 製造工程をチェックリスト化する**（設計書確定後、画面・処理ごとに依存順の工程へ分解する）
- [ ] **4. 製造する**（工程は3で確定）
