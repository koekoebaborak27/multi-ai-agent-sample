...
| **マスタ分類の見直し** | **4 / 4** |
| **契約先・契約の管理画面整備** | **6 / 6** |
| **パスワード再発行機能（要件・設計3＋メール送信設定4＋製造6＋テスト2＋本番1）** | **15 / 16** |
...
**パスワード再発行機能はメール送信設定・5-工程1〜6・6〜7番（単体テスト仕様書作成・画面操作テスト）まで完了している**（2026-08-21・22）。各項目の履歴リンクは [パスワード再発行機能](#パスワード再発行機能) のチェックリストを参照。`prisma migrate dev` がエージェントの非対話シェルで使えない問題への対処は [`prisma-cli.md`](notes/prisma-cli.md) を参照。

**次にやることは、[パスワード再発行機能](#パスワード再発行機能) の 8番（本番へ反映する）に着手すること。** 着手前にユーザーへ確認する。
...
- [x] **6. 単体テスト仕様書を作成する**（2026-08-22。PWR-01/PWR-02/EML-01/EML-02/利用者メールアドレス管理の5仕様書）→ [履歴](history/2026-08-w3.md#2026-08-22-パスワード再発行機能の単体テスト仕様書作成と画面操作テストを完了工程6後半)
- [x] **7. 画面操作テストを実施する**（2026-08-22。Playwrightで計30ケース全て成功）→ [履歴](history/2026-08-w3.md#2026-08-22-パスワード再発行機能の単体テスト仕様書作成と画面操作テストを完了工程6後半)
- [ ] **8. 本番へ反映する**（マイグレーション適用・**Secret Manager へ `smtp-password` を登録**し Cloud Run の環境変数を設定・本番での送信確認。手順は [`infra_design_09_メール送信.md` §09.1.7](../specs/99_infra/infra_design_09_メール送信.md#0917-本番cloud-runに設定する)）
...
| 項目 | 状態 |
| ------------ | ----------------------------------------------------------------------------------------------- |
| 作業ブランチ | `feat/password-reset-db-mail`（[PR #27](https://github.com/koekoebaborak27/multi-ai-agent-sample/pull/27)、未マージ）。パスワード再発行機能の工程1〜6・単体テスト仕様書・環境修正を1つのPRにまとめて積んでいる。`git log --oneline -1`で最新の反映内容を確認できる |
...
| 直近の検証 | 2026-08-22、パスワード再発行機能の6・7番でPlaywright（`--headed`）により5仕様書・計30ケース全て成功（PWR-01: 5件・PWR-02: 8件・EML-01: 6件・EML-02: 5件・利用者メールアドレス管理: 6件）。テストで作成した専用データは全て自動削除済みで、既存の運用データへの影響なし |
