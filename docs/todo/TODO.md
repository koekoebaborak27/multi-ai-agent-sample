...
**次のセッションでは、[マスタ分類の編集・削除に警告アラートを追加する](#マスタ分類の編集削除に警告アラートを追加する) の1番（設計書の見直し）から着手する**（廃止方針は同日中に撤回済み。→ [履歴](history/2026-08-w3.md#2026-08-23-マスタ分類の登録編集削除機能を廃止する方針を撤回し警告アラート方式へ変更)）。そのあとに[公開用テンプレートの切り出し](#公開用テンプレートの切り出し) に着手する（2026-08-23 に方針を確定済み。→ [履歴](history/2026-08-w3.md#2026-08-23-公開用テンプレートの切り出し方針を確定)）。最初にやるのは、今のフォルダを別の場所へコピーして Git 履歴なしの新品状態にすること。
...
- [ ] 1. 設計書を見直す（`docs/specs/02_basic-design/master/`22〜24番。警告アラートの対象分類（契約先分類・契約分類など、システムが参照する分類をどう判定するか）・表示文言・表示箇所（MST-07の編集・削除ボタン、MST-09の実行ボタン等）を確定する）
- [ ] 2. 実装する（対象ボタン押下時に警告アラートを表示する。既存の確認ダイアログ・確認画面へ文言を追加する形を想定）
- [ ] 3. 関連するテスト（単体テスト仕様書・Playwright証跡）を更新する
- [ ] 4. ローカル・本番で動作確認する
