...
**工程 18「単体テスト実施」に着手する。** 工程17（CSVの状態確認と受け取り）が完了し、CSVダウンロードは依頼 → 生成 → 受け取りまで**画面からの一連の操作が通るようになった**（MST-01・MST-06のどちらも実機確認済み）。工程12から持ち越していた §12.1〜§12.7 の単体テスト仕様書化・MST-01〜MST-10 のブラウザ操作も、この工程18でまとめて行う。スキル（`markdown-unit-test-spec` → `playwright-evidence-test`）を使い、画面ごとに仕様書を作成してから画面操作テストを実行する。エビデンス（スクリーンショット等）は `.gitignore` でリポジトリ管理の対象外とする。ADMIN パスワードは `prisma/seed.ts` のデフォルト値 `Admin@123` を試す（`.env` に `SEED_ADMIN_PASSWORD` があれば上書きされている）。
