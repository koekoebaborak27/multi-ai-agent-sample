...
**工程 15「CSV の依頼処理を実装する」に着手する。** 工程14（`MasterExport` モデルとCSV生成処理）は完了した。工程12の残り（§12.1〜§12.7 の単体テスト仕様書化、MST-01〜MST-10 のブラウザ操作）は工程18へ持ち越している。ADMIN パスワードは `prisma/seed.ts` のデフォルト値 `Admin@123` を試す（`.env` に `SEED_ADMIN_PASSWORD` があれば上書きされている）。

最初に打つコマンド:

```powershell
rg -n "^### 13\.5" docs/specs/02_basic-design/master/30_CSVダウンロード.md   # §13.5.1 が依頼処理（Server Action）の対象節
...
| 項目 | 状態 |
| ------------ | ----------------------------------------------------------------------------------------------- |
| 作業ブランチ | `feat/master-management`。工程 11 まで push 済み、CI 成功。工程 13・14 はローカル実装済みで未コミット |
...
| 直近の検証 | 2026-08-12、工程14実装後の状態で `lint` / `format:check` / `typecheck` / 11 ファイル 161 テスト / `build` が成功 |
