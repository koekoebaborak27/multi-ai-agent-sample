...
| **マスタ分類の見直し** | **4 / 4** |
| **契約先・契約の管理画面整備** | **6 / 6** |
| **パスワード再発行機能（要件・設計3＋メール送信設定4＋製造6＋テスト1＋本番1）** | **9 / 15** |
...
**5-工程1（データベースの変更）・工程2（メール送信の共通の仕組み）は完了している**（2026-08-21）→ [履歴](history/2026-08-w3.md#2026-08-21-パスワード再発行機能のデータベース変更を実装工程1)・[履歴](history/2026-08-w3.md#2026-08-21-パスワード再発行機能のメール送信の共通の仕組みを実装工程2)。`prisma migrate dev` がエージェントの非対話シェルで使えない問題への対処は [`prisma-cli.md`](notes/prisma-cli.md) を参照。

**次にやることは、[パスワード再発行機能](#パスワード再発行機能) の 5-工程3（利用者管理をメールアドレスに対応させる）に着手すること。** 設計は [`20_メールアドレス管理.md` §20.1](../specs/02_basic-design/password-reset/20_メールアドレス管理.md#201-管理者による登録と変更) にある。まず `git status --porcelain` で未コミット差分がないことを確認する（**2026-08-21 の工程1・工程2が未コミットで残っている場合は先にコミットする**）。
...
| 直近の検証 | 2026-08-21、パスワード再発行機能の5-工程1・工程2で `pnpm lint`/`format:check`/`typecheck`/`test`（310件）/`build` すべて成功。画面からの呼び出しがまだ無いためブラウザ確認は未実施（工程4〜6で実施） |
