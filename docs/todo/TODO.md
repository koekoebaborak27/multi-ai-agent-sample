...
**期限のある残作業は無い。** テンプレートの土台は本番稼働まで到達した。次のセッションは [残っているタスク](#残っているタスク) から選ぶか、`src/modules/` へ案件固有の業務ドメインを足す段階へ進む。

宿題から着手するなら、次の順で価値が高い。
...
| ファイル                                                | 内容                                                 |
| --------------------------------------------------- | -------------------------------------------------- |
| [`TODO_補足.md`](TODO_補足.md)                          | Supabase / Cloud Run の設定値・手順・落とし穴。**冒頭に節ごとの目次がある** |
