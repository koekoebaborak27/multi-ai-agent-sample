...
**公開用テンプレート切り出し前の準備の1番は2026-08-27に決着した**（現状維持を選択、→ [完了済みの作業](#完了済みの作業)）。**次のセッションでは、[公開用テンプレート切り出し前の準備](#公開用テンプレート切り出し前の準備)の2番（マイグレーションの自動化を検討する）に着手する。** 判断材料は [`prisma_operations.md`](../prisma_operations.md) と [`docs/specs/99_infra/` §10.1.2](../specs/99_infra/infra_design_10_構築後の運用.md#1012-データベースの構造を変更する)。**[公開用テンプレートの切り出し](#公開用テンプレートの切り出し)はこの準備が終わってから着手する。** 最初に `git status --porcelain` で未コミット差分を確認する。
...
[公開用テンプレートの切り出し](#公開用テンプレートの切り出し)に着手する前に決めておきたい運用上の論点2つ。

- [x] **1. ドキュメントのみの変更で Cloud Build を走らせない仕組みを入れるか決める**（2026-08-27、現状維持を選択）→ [履歴](history/2026-08-w4.md#2026-08-27-ドキュメントのみの変更でcloud-buildを走らせない仕組みの導入を見送り公開用テンプレート切り出し前の準備1番)
- [ ] 2. マイグレーションの自動化を検討する — 当面はローカルからの手動 `prisma migrate deploy`。理由と手順は [`prisma_operations.md`](../prisma_operations.md)、[`docs/specs/99_infra/` §09.1.2](../specs/99_infra/infra_design_10_構築後の運用.md#1012-データベースの構造を変更する)
