...
**工程 16「CSV を生成する worker を実装する」に着手する。** 工程15（CSVの依頼処理・Server Action）は完了した。ローカルではまだ worker がジョブを購読していないため、依頼してもCSVは生成されない（工程16で解消する）。工程12の残り（§12.1〜§12.7 の単体テスト仕様書化、MST-01〜MST-10 のブラウザ操作）は工程18へ持ち越している。ADMIN パスワードは `prisma/seed.ts` のデフォルト値 `Admin@123` を試す（`.env` に `SEED_ADMIN_PASSWORD` があれば上書きされている）。

最初に打つコマンド:

```powershell
rg -n "^### 13\.5\.2|^### 13\.7" docs/specs/02_basic-design/master/30_CSVダウンロード.md   # §13.5.2 が生成（worker）、§13.7 が実行方式の対象節
...
| 項目 | 状態 |
| ------------ | ----------------------------------------------------------------------------------------------- |
| 作業ブランチ | `feat/master-management`。工程 14 まで push 済み、CI 成功。工程 15 はローカル実装済みで未コミット |
...
| 直近の検証 | 2026-08-12、工程15実装後の状態で `lint` / `format:check` / `typecheck` / 11 ファイル 171 テスト / `build` が成功 |
