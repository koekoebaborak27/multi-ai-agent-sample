...
| **公開用テンプレートの切り出し前準備（ER図・画面遷移図・セキュリティテスト）**                     | **3 / 4**   |
| **公開用テンプレートの切り出し**                                            | **0 / 8**   |
...
**次のセッションは、[公開用テンプレートの切り出し前準備](#公開用テンプレートの切り出し前準備)の4番（セキュリティテストの仕様書を作り、テストする）から着手する。** 最初に `git status --porcelain` で未コミット差分を確認する。
...
- [x] **1. 図の変換方法を決める**（2026-08-28、`npm install -g`ではなく公式Dockerイメージ`minlag/mermaid-cli`方式に決定。`docs/diagrams.md`を全面改訂）→ [履歴](history/2026-08-w4.md#2026-08-28-er図をdocker方式のmermaidで作成しdb定義変更時の手順書を新設)
- [x] **2. mermaidの`erDiagram`でER図を作成する**（2026-08-28、[`docs/specs/98_db/db_spec.md`](../specs/98_db/db_spec.md)として`foundation_plan.md`から切り出し）→ [履歴](history/2026-08-w4.md#2026-08-28-er図をdocker方式のmermaidで作成しdb定義変更時の手順書を新設)
- [x] **3. mermaidで画面遷移図を作成する**（2026-08-28、[`docs/specs/02_basic-design/画面遷移図.md`](../specs/02_basic-design/画面遷移図.md)として新設）→ [履歴](history/2026-08-w4.md#2026-08-28-mermaidで画面遷移図を作成する公開用テンプレート切り出し前の準備3番)
- [ ] 4. セキュリティテストの仕様書を作り、テストする
...
| 直近の検証 | 2026-08-27、ログイン画面・トップ画面の画面操作テストで発見したアカウントロック時のメッセージ表示不具合（`src/modules/auth/actions.ts`）を修正し、単体テスト全389件・Lint（既存警告1件のみ）・型チェックが成功（Dockerコンテナ内で確認）。テストコード（`e2e/auth/`・`e2e/dashboard/`）と不具合修正は未コミット |
