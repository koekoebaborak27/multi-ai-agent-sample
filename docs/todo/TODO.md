...
| **お知らせ管理機能（設計1＋実装1＋テスト仕様書1＋テスト実施1）**                          | **0 / 4**   |
| **公開用テンプレートの切り出し**                                            | **0 / 8**   |
...
**次のセッションでは、[お知らせ管理機能](#お知らせ管理機能) の基本設計書の作成に着手する**（要件定義は2026-08-24に確定済み。→ [要件定義](../specs/01_requirements/news/01_お知らせ管理.md)、[履歴](history/2026-08-w4.md#2026-08-24-お知らせ管理機能の要件定義を実施)）。[公開用テンプレートの切り出し](#公開用テンプレートの切り出し)（2026-08-23 に方針を確定済み。→ [履歴](history/2026-08-w3.md#2026-08-23-公開用テンプレートの切り出し方針を確定)）は、この後に着手する。
...
ダッシュボード（トップ画面）に表示する「お知らせ」を、`ADMIN`・`OPERATOR`が自分たちで登録・更新・削除できるようにする。マスタ機能の進め方（設計→製造→単体テスト）に倣う。要件定義は2026-08-24に1問1答形式で確定した（→ [要件定義](../specs/01_requirements/news/01_お知らせ管理.md)）。

- [ ] 1. 基本設計書を作成する（`docs/specs/02_basic-design/news/` へ、マスタ・パスワード再発行と同じ粒度で機能ごとに分割）
- [ ] 2. 実装する（Prisma `Announcement` モデルの変更、お知らせ管理画面・お知らせ一覧・トップ画面のお知らせウィジェットの3画面）
- [ ] 3. 単体テスト仕様書を作成する
- [ ] 4. 単体テスト（画面操作テスト）を実施する
