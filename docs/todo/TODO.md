...
**公開用テンプレート切り出し前の準備は2026-08-27に2番とも決着した**（→ [完了済みの作業](#完了済みの作業)）。**次のセッションでは、[公開用テンプレートの切り出し](#公開用テンプレートの切り出し)の1番（今のフォルダを別の場所へコピーする）から着手する。** 最初に `git status --porcelain` で未コミット差分を確認する。
