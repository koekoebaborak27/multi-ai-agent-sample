...
| **不足ドキュメントの作成（棚卸し1＋要件定義・基本設計4＋単体テスト仕様書1＋テスト実施1）**             | **6 / 7**   |
| **公開用テンプレートの切り出し**                                            | **0 / 8**   |
...
**[不足ドキュメントの作成](#不足ドキュメントの作成)は2026-08-27に6番まで完了した**（対象範囲の確定、ログイン画面・トップ画面・マスタ機能・契約先契約管理の要件定義書、ログイン画面・トップ画面の基本設計書、ログイン画面・トップ画面の単体テスト仕様書3件。→ [履歴](history/2026-08-w4.md#2026-08-27-ログイン画面トップ画面の単体テスト仕様書を作成不足ドキュメントの作成6番)）。**次のセッションでは、7番（6で作成した単体テスト仕様書3件を使ったPlaywright画面操作テスト）に着手する。** 最初に `git status --porcelain` で未コミット差分を確認する。画面操作テストには `playwright-evidence-test` スキルが使える。不足ドキュメントの作成が完了したら、[公開用テンプレートの切り出し](#公開用テンプレートの切り出し)（2026-08-23 に方針を確定済み。→ [履歴](history/2026-08-w3.md#2026-08-23-公開用テンプレートの切り出し方針を確定)）へ進む。
...
要件定義書（[`docs/specs/01_requirements/`](../specs/01_requirements/)）・基本設計書（[`docs/specs/02_basic-design/`](../specs/02_basic-design/)）・単体テスト仕様書（[`docs/test/unit/spec/`](../test/unit/spec/)）のいずれかが無い機能・画面を作成し、単体テスト仕様書を使った画面テストを実行する。2026-08-26時点の棚卸し結果: ログイン画面・トップ画面（ダッシュボード）は3種類とも無し。マスタ機能・契約先契約管理画面整備（[`master/`](../specs/02_basic-design/master/README.md)・[`party-contract/`](../specs/02_basic-design/party-contract/README.md)）は基本設計書のみあり要件定義書が無し（お知らせ管理機能は要件定義書・基本設計書・単体テスト仕様書のすべてが揃っているため対象外）。

- [x] **1. 対象範囲を最終確定する**（2026-08-27、追加の漏れなしを確認）→ [履歴](history/2026-08-w4.md#2026-08-27-ログイン画面トップ画面の要件定義書基本設計書を作成不足ドキュメントの作成対象範囲確定23番)
- [x] **2. ログイン画面の要件定義書・基本設計書を作成する**（2026-08-27）→ [`01_requirements/login/`](../specs/01_requirements/login/01_ログイン.md)・[`02_basic-design/login/`](../specs/02_basic-design/login/README.md)
- [x] **3. トップ画面（ダッシュボード）の要件定義書・基本設計書を作成する**（2026-08-27）→ [`01_requirements/dashboard/`](../specs/01_requirements/dashboard/01_トップ画面.md)・[`02_basic-design/dashboard/`](../specs/02_basic-design/dashboard/README.md)
- [x] **4. マスタ機能の要件定義書を作成する**（2026-08-27）→ [`01_requirements/master/01_マスタ管理.md`](../specs/01_requirements/master/01_マスタ管理.md)
- [x] **5. 契約先・契約管理画面整備の要件定義書を作成する**（2026-08-27）→ [`01_requirements/party-contract/01_契約先契約管理.md`](../specs/01_requirements/party-contract/01_契約先契約管理.md)
- [x] **6. ログイン画面・トップ画面の単体テスト仕様書を作成する**（2026-08-27）→ [`auth/UT_10_ログイン画面.md`](../test/unit/spec/auth/UT_10_ログイン画面.md)・[`auth/UT_11_初回パスワード変更画面.md`](../test/unit/spec/auth/UT_11_初回パスワード変更画面.md)・[`dashboard/UT_10_トップ画面.md`](../test/unit/spec/dashboard/UT_10_トップ画面.md)
- [ ] 7. 6で作成した単体テスト仕様書を使って画面テスト（Playwright）を実行する
...
| 直近の検証 | 2026-08-27、お知らせ管理機能の画面操作テストで発見したカテゴリ選択欄のID重複・削除完了トーストの競合状態を修正し、単体テスト全389件・Lint（既存警告1件のみ）・型チェック・本番ビルド・整形確認が成功 |
