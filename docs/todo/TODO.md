...
| **公開用テンプレートの切り出し前準備（ER図・画面遷移図・セキュリティテスト）**                     | **2 / 4**   |
| **公開用テンプレートの切り出し**                                            | **0 / 8**   |
...
**次のセッションは、[公開用テンプレートの切り出し前準備](#公開用テンプレートの切り出し前準備)の3番（mermaidで画面遷移図を作成する）から着手する。** 最初に `git status --porcelain` で未コミット差分を確認する。
...
- [ ] 3. mermaidで画面遷移図を作成する（[`docs/diagrams.md`](../diagrams.md)の手順で`.mmd`→SVG化）
