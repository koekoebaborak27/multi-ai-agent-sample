...
**工程 12 でマスタ機能を検証する。** 設計書 §12.1〜§12.7 の単体テストに加え、`lint` / `format:check` / `typecheck` / `test` / `build` と MST-01〜MST-10 のブラウザ操作を通す。ローカル管理者パスワードが不明なため ADMIN でのブラウザ確認が積み残しになっており、これが前提条件になる。工程11（マスタ分類削除）はローカルで実装済みだが**未コミット**であることに注意する。

最初に打つコマンド:

```powershell
git status --porcelain                                   # 工程11の変更が残っていることを確認
git diff --stat                                          # 変更ファイルの一覧
rg -n "^## 12\.|^### 12\." docs/specs/02_basic-design/master/*.md  # §12 の単体テスト観点は画面ごとに分散している
```

手元の状態を確認するコマンド:

```powershell
git log --oneline -1                                     # 現在のコミット（工程11はまだ含まれない）
git status --porcelain                                   # 工程11分の未コミット差分が出る
...
| 項目 | 状態 |
| ------------ | ----------------------------------------------------------------------------------------------- |
| 作業ブランチ | `feat/master-management`。工程 10 まで push 済み、CI 成功。**工程 11 はローカル実装済みで未コミット** |
...
| 直近の検証 | 工程 11 完了時点で `lint` / `format:check` / `typecheck` / 9 ファイル 152 テスト / `build` が成功（コミット前） |
