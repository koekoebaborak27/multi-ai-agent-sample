...
| **不足ドキュメントの作成（棚卸し1＋要件定義・基本設計4＋単体テスト仕様書1＋テスト実施1）**             | **3 / 7**   |
| **公開用テンプレートの切り出し**                                            | **0 / 8**   |
...
**[不足ドキュメントの作成](#不足ドキュメントの作成)は2026-08-27に4番まで完了した**（対象範囲の確定、ログイン画面・トップ画面の要件定義書・基本設計書。→ [履歴](history/2026-08-w4.md#2026-08-27-ログイン画面トップ画面の要件定義書基本設計書を作成不足ドキュメントの作成対象範囲確定23番)）。**次のセッションでは、4番（マスタ機能の要件定義書）・5番（契約先・契約管理画面整備の要件定義書）に着手する。** 最初に `git status --porcelain` で未コミット差分を確認する。作成済みのマスタ・契約先契約の基本設計書（`docs/specs/02_basic-design/master/`・`party-contract/`）を土台に、実装済みの権限・画面構成を読み解いて要件定義書を作成する（既存のnews・password-resetのような新規壁打ちではなく、ログイン・トップ画面と同様の「実装済み機能の後追い文書化」であることに注意）。6番（ログイン画面・トップ画面の単体テスト仕様書の作成）・7番（Playwright実行）は5番までの完了後に着手する。不足ドキュメントの作成が完了したら、[公開用テンプレートの切り出し](#公開用テンプレートの切り出し)（2026-08-23 に方針を確定済み。→ [履歴](history/2026-08-w3.md#2026-08-23-公開用テンプレートの切り出し方針を確定)）へ進む。
