...
| **マスタ分類の見直し** | **4 / 4** |
| **契約先・契約の管理画面整備** | **6 / 6** |
| **パスワード再発行機能（要件・設計3＋メール送信設定4＋製造6＋テスト1＋本番1）** | **10 / 15** |
...
**5-工程1〜3（データベースの変更・メール送信の共通の仕組み・利用者管理のメールアドレス対応）は完了している**（2026-08-21）→ [履歴](history/2026-08-w3.md#2026-08-21-パスワード再発行機能のデータベース変更を実装工程1)・[履歴](history/2026-08-w3.md#2026-08-21-パスワード再発行機能のメール送信の共通の仕組みを実装工程2)・[履歴](history/2026-08-w3.md#2026-08-21-パスワード再発行機能の利用者管理をメールアドレスに対応させた工程3)。`prisma migrate dev` がエージェントの非対話シェルで使えない問題への対処は [`prisma-cli.md`](notes/prisma-cli.md) を参照。

**次にやることは、[パスワード再発行機能](#パスワード再発行機能) の 5-工程4（再発行の申請画面 PWR-01 とログイン画面のリンクを実装する）に着手すること。** 設計は [`10_再発行申請.md`](../specs/02_basic-design/password-reset/10_再発行申請.md) にある。
...
| 直近の検証 | 2026-08-21、パスワード再発行機能の5-工程3で `pnpm lint`/`format:check`/`typecheck`/`test`（326件）/`build` すべて成功。利用者管理画面（`/admin/users`）でメール登録・重複エラー・新規登録の必須バリデーションをローカルブラウザで確認済み |
...
| メール送信 | 送信の共通窓口 `src/shared/mail/`（`sendMail`）を実装済み（2026-08-21）。まだどの画面・処理からも呼ばれていない（呼び出しは工程4〜6）。ローカルは送信専用の Gmail アカウントとアプリパスワードを `.env.local` に設定し、実送信を確認済み。**本番（Cloud Run / Secret Manager）は未設定**。`.env.example` の既定は `MAIL_TRANSPORT=console`（送らずログへ出すだけ）のため、設定なしでも開発できる |
