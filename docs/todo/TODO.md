...
| **マスタ分類の見直し** | **4 / 4** |
| **契約先・契約の管理画面整備** | **6 / 6** |
| **パスワード再発行機能（要件・設計3＋メール送信設定4＋製造6＋テスト1＋本番1）** | **13 / 15** |
...
**5-工程1〜5（データベースの変更・メール送信の共通の仕組み・利用者管理のメールアドレス対応・再発行の申請画面とログイン画面のリンク・パスワードの再設定画面）は完了している**（2026-08-21）→ [履歴](history/2026-08-w3.md#2026-08-21-パスワード再発行機能のデータベース変更を実装工程1)・[履歴](history/2026-08-w3.md#2026-08-21-パスワード再発行機能のメール送信の共通の仕組みを実装工程2)・[履歴](history/2026-08-w3.md#2026-08-21-パスワード再発行機能の利用者管理をメールアドレスに対応させた工程3)・[履歴](history/2026-08-w3.md#2026-08-21-パスワード再発行機能の再発行申請画面とログイン画面のリンクを実装工程4)・[履歴](history/2026-08-w3.md#2026-08-21-パスワード再発行機能のパスワード再設定画面を実装工程5)。`prisma migrate dev` がエージェントの非対話シェルで使えない問題への対処は [`prisma-cli.md`](notes/prisma-cli.md) を参照。

**5-工程6（本人によるメールアドレス変更 EML-01・EML-02）は完了している**（2026-08-21）→ [履歴](history/2026-08-w3.md#2026-08-21-パスワード再発行機能の本人によるメールアドレス変更を実装工程6)。

**次にやることは、[パスワード再発行機能](#パスワード再発行機能) の 6番（単体テスト仕様書を作成し、画面操作テストを実施する）に着手すること。** 着手前にユーザーへ確認する。
...
- [x] **5. 製造する**（2026-08-21。工程1〜6完了）
  ...
  - [x] 工程5. パスワード再設定画面（PWR-02）を実装する（ロック解除・変更完了メールを含む。[`11_パスワード再設定.md`](../specs/02_basic-design/password-reset/11_パスワード再設定.md)）（2026-08-21）→ [履歴](history/2026-08-w3.md#2026-08-21-パスワード再発行機能のパスワード再設定画面を実装工程5)
  - [x] 工程6. 本人によるメールアドレス変更（EML-01・EML-02）を実装する（確認用URLで確かめる。[`20_メールアドレス管理.md` §20.2](../specs/02_basic-design/password-reset/20_メールアドレス管理.md#202-本人による変更eml-01)）（2026-08-21）→ [履歴](history/2026-08-w3.md#2026-08-21-パスワード再発行機能の本人によるメールアドレス変更を実装工程6)
...
| 項目 | 状態 |
| ------------ | ----------------------------------------------------------------------------------------------- |
| 作業ブランチ | `feat/password-reset-db-mail`（[PR #27](https://github.com/koekoebaborak27/multi-ai-agent-sample/pull/27)、未マージ）。パスワード再発行機能の工程1〜6を1つのPRにまとめて積んでいる。`git log --oneline -1`で最新の反映内容を確認できる |
...
| 直近の検証 | 2026-08-21、パスワード再発行機能の5-工程6で `pnpm lint`/`format:check`/`typecheck`/`test`（367件）/`build` すべて成功。`/settings/email`・`/settings/email/confirm/[token]`をローカルブラウザで確認: 現在のアドレスと同じ入力を拒否→他人が使用中のアドレスを拒否→申し込み成功→ログのURLから確認画面を開いて変更完了→`User.email`とお知らせメール送信をDB・ログで確認→使用済みURLの再アクセスで「開けない」表示→未ログインで確認URLを開くと`/login`へ誘導されることを確認済み |
...
| メール送信 | 送信の共通窓口 `src/shared/mail/`（`sendMail`）を実装済み（2026-08-21）。再発行申請画面（工程4）・パスワード再設定画面（工程5、`password-changed`）・メールアドレス変更申し込みと確定（工程6、`email-change-confirm`/`email-changed`）から呼び出す状態になり、4種類すべての文面を使い切った。ローカルは送信専用の Gmail アカウントとアプリパスワードを `.env.local` に設定し、実送信を確認済み。**本番（Cloud Run / Secret Manager）は未設定**。`.env.example` の既定は `MAIL_TRANSPORT=console`（送らずログへ出すだけ）のため、設定なしでも開発できる |
