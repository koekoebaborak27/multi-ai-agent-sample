...
| **マスタ分類の見直し** | **4 / 4** |
| **契約先・契約の管理画面整備** | **4 / 5（要件定義・設計・工程分解・製造〈工程1〜12〉が完了。残るは単体テスト仕様書の作成と画面操作テスト）** |
...
**契約先・契約の管理画面整備の製造（工程1〜12）が完了した**（2026-08-19）。契約先・契約それぞれの検索一覧・新規登録・確認画面・詳細・更新・削除を実装し、ローカルブラウザで一通り動作確認済み。詳細は [履歴](history/2026-08-w3.md#2026-08-19-契約先契約の管理画面整備の製造を実施工程1〜12) を参照。

**次にやることは、[契約先・契約の管理画面整備](#契約先契約の管理画面整備)の5番（単体テスト仕様書を作成し、画面操作テストを実施する）から着手すること。**
...
| 直近の検証 | 2026-08-19、契約先・契約の管理画面整備（工程1〜12）を実装し、`pnpm typecheck`/`test`（294件）/`lint`/`format:check` すべて成功。ローカルブラウザで契約先・契約それぞれの検索一覧・新規登録・確認画面・詳細・更新・削除（契約先は紐づく契約がある場合のエラー表示を含む）を一通り確認済み |
