...
**工程 18「単体テスト実施」に着手する。** 工程17（CSVの状態確認と受け取り）が完了し、CSVダウンロードは依頼 → 生成 → 受け取りまで**画面からの一連の操作が通るようになった**（MST-01・MST-06のどちらも実機確認済み）。工程12から持ち越していた §12.1〜§12.7 の単体テスト仕様書化・MST-01〜MST-10 のブラウザ操作も、この工程18でまとめて行う。スキル（`create-unit-test-spec` → `playwright-evidence-test`）を使い、画面ごとに仕様書を作成してから画面操作テストを実行する。`@playwright/test`（1.62.1）と chromium ブラウザ本体は導入済み（2026-08-13）→ [履歴](history/2026-08-w2.md#2026-08-13-playwrightを導入)。エビデンス（スクリーンショット等）は `.gitignore` でリポジトリ管理の対象外とする。ADMIN パスワードは `prisma/seed.ts` のデフォルト値 `Admin@123` を試す（`.env` に `SEED_ADMIN_PASSWORD` があれば上書きされている）。
...
- [ ] **18. 単体テスト実施** ―スキルを使って単体テスト仕様書を対応する設計書ファイル単位（1機能1仕様書）で作成し、画面テストを行ってエビデンスを作成する。エビデンスについては.gitignoreでリポジトリ管理の対象外とする。（仕様書 1 / 10 作成済み・画面テストは 0 / 10）
  - [ ] 18-1. マスタ分類一覧（MST-06） → `UT_20_マスタ分類一覧.md`
...
| ブラウザ検証 | ADMIN で CSV ダウンロード（MST-01 / MST-06、工程17分）は実機確認済み。工程 6〜11 分のブラウザ操作は未実施のまま。**工程 18 の全画面検証に含める** |
| 直近の検証 | 2026-08-12、工程17実装後の状態で `lint` / `format:check` / `typecheck` / 13 ファイル 184 テストが成功。ローカル docker compose で CSV ダウンロード（依頼→生成→受け取り）を MST-01・MST-06 の両方で実機確認済み |
