...
| **お知らせ管理機能（設計1＋実装9＋テスト仕様書1＋テスト実施1）**                          | **11 / 12** |
...
**次のセッションでは、[お知らせ管理機能](#お知らせ管理機能) の工程12（単体テストの画面操作テスト）に着手する**（工程11の仕様書作成は2026-08-27に完了済み。→ [履歴](history/2026-08-w4.md#2026-08-27-お知らせ管理機能の単体テスト仕様書を作成)）。最初に `git status --porcelain` で未コミット差分を確認する。お知らせ管理機能の完了後は [不足ドキュメントの作成](#不足ドキュメントの作成)（2026-08-26 追加）に着手し、その後に [公開用テンプレートの切り出し](#公開用テンプレートの切り出し)（2026-08-23 に方針を確定済み。→ [履歴](history/2026-08-w3.md#2026-08-23-公開用テンプレートの切り出し方針を確定)）へ進む。
...
- [x] 7. **お知らせ管理一覧の検索・並び替えを実装する**（2026-08-25）→ [履歴](history/2026-08-w4.md#2026-08-25-お知らせ管理機能のお知らせ管理一覧の検索並び替え工程7を完了)
- [x] 8. **お知らせ登録を実装する**（2026-08-25）→ [履歴](history/2026-08-w4.md#2026-08-25-お知らせ管理機能のお知らせ登録工程8を完了)
- [x] **9. お知らせ更新を実装する**（2026-08-25）→ [履歴](history/2026-08-w4.md#2026-08-25-お知らせ管理機能のお知らせ更新工程9を完了)
- [x] **10. お知らせ削除を実装する**（2026-08-25）→ [履歴](history/2026-08-w4.md#2026-08-25-お知らせ管理機能のお知らせ削除工程10を完了)
- [x] 11. **単体テスト仕様書を作成する**（2026-08-27）→ [履歴](history/2026-08-w4.md#2026-08-27-お知らせ管理機能の単体テスト仕様書を作成)
- [ ] 12. 単体テスト（画面操作テスト）を実施する

## 不足ドキュメントの作成

要件定義書（[`docs/specs/01_requirements/`](../specs/01_requirements/)）・基本設計書（[`docs/specs/02_basic-design/`](../specs/02_basic-design/)）・単体テスト仕様書（[`docs/test/unit/spec/`](../test/unit/spec/)）のいずれかが無い機能・画面を作成し、単体テスト仕様書を使った画面テストを実行する。2026-08-26時点の棚卸し結果: ログイン画面・トップ画面（ダッシュボード）は3種類とも無し。マスタ機能・契約先契約管理画面整備（[`master/`](../specs/02_basic-design/master/README.md)・[`party-contract/`](../specs/02_basic-design/party-contract/README.md)）は基本設計書のみあり要件定義書が無し（お知らせ管理機能は対応中のため対象外。→ [お知らせ管理機能](#お知らせ管理機能)）。

- [ ] 1. 対象範囲を最終確定する（上記棚卸し結果を再確認し、他に漏れがないか3ディレクトリを突き合わせる）
- [ ] 2. ログイン画面の要件定義書・基本設計書を作成する
- [ ] 3. トップ画面（ダッシュボード）の要件定義書・基本設計書を作成する
- [ ] 4. マスタ機能の要件定義書を作成する
- [ ] 5. 契約先・契約管理画面整備の要件定義書を作成する
- [ ] 6. ログイン画面・トップ画面の単体テスト仕様書を作成する
