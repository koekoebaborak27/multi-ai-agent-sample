...
**工程 17「CSV の状態確認と受け取りを実装する」に着手する。** 工程16（CSVを生成するworker）は完了し、ローカルの docker compose worker で依頼→生成（`READY`）まで実際に確認済み。ただし状態確認・受け取りの Route Handler（§13.5.3）とダウンロードボタン（§13.3）がまだ無いため、**画面からの一連の操作はまだ通らない**（工程17で解消する）。工程12の残り（§12.1〜§12.7 の単体テスト仕様書化、MST-01〜MST-10 のブラウザ操作）は工程18へ持ち越している。ADMIN パスワードは `prisma/seed.ts` のデフォルト値 `Admin@123` を試す（`.env` に `SEED_ADMIN_PASSWORD` があれば上書きされている）。

最初に打つコマンド:

```powershell
rg -n "^### 13\.3|^### 13\.5\.3|^### 13\.9|^### 13\.10" docs/specs/02_basic-design/master/30_CSVダウンロード.md   # §13.3 が導線、§13.5.3 が状態確認・受け取り、§13.9 が保持と削除、§13.10 が待ち時間とエラーの対象節
...
| 項目 | 状態 |
| ------------ | ----------------------------------------------------------------------------------------------- |
| 作業ブランチ | `feat/master-management`。工程 16 まで push 済み、CI 成功 |
...
| 直近の検証 | 2026-08-12、工程16実装後の状態で `lint` / `format:check` / `typecheck` / 11 ファイル 171 テストが成功。ローカル worker で依頼→生成（常駐・単発の両モード）も実機確認済み。push後の CI（Format check 含む）も成功 |
