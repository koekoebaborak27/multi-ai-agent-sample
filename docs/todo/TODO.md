...
**工程 18「単体テスト実施」を継続する。18-1（マスタ分類一覧）が完了し、次は 18-2（マスタ分類の新規登録と確認）から。** スキル（`create-unit-test-spec` → `playwright-evidence-test`）を使い、画面ごとに仕様書を作成してから画面操作テストを実行する。工程12から持ち越していた §12.1〜§12.7 の単体テスト仕様書化・MST-01〜MST-10 のブラウザ操作も、この工程18でまとめて行う。ログインは `.env.example` の自動テスト用アカウント（`SEED_ADMIN_*` / `SEED_OPERATOR_*` / `SEED_VIEWER_*`）を使う（OPERATOR・VIEWERはDBに投入済みを確認済み）。エビデンス（スクリーンショット等）は `.gitignore` でリポジトリ管理の対象外とする。

最初に打つコマンド:

```powershell
rg -n "^### 12\.1|^### 12\.2|^### 12\.3|^### 12\.4|^### 12\.5|^### 12\.6|^### 12\.7" docs/specs/02_basic-design/master/*.md   # 単体テスト仕様書化の対象節（工程12から持ち越し）
...
- [ ] **18. 単体テスト実施** ―スキルを使って単体テスト仕様書を対応する設計書ファイル単位（1機能1仕様書）で作成し、画面テストを行ってエビデンスを作成する。エビデンスについては.gitignoreでリポジトリ管理の対象外とする。（仕様書 2 / 10 作成済み・画面テストは 1 / 10）
  - [x] 18-1. マスタ分類一覧（MST-06） → `UT_20_マスタ分類一覧.md`（2026-08-13）→ [履歴](history/2026-08-w2.md#2026-08-13-マスタ分類一覧の単体テスト仕様書作成と画面操作テストを実施)
  - [ ] 18-2. マスタ分類の新規登録と確認（MST-08 / MST-09） → `UT_22_マスタ分類新規登録.md`
  - [ ] 18-3. マスタ分類詳細（MST-07） → `UT_21_マスタ分類詳細.md`
  - [ ] 18-4. マスタ分類更新（MST-10 / MST-09） → `UT_23_マスタ分類更新.md`
  - [ ] 18-5. マスタ分類削除（MST-07） → `UT_24_マスタ分類削除.md`
  - [ ] 18-6. マスタ検索一覧（MST-01） → `UT_10_マスタ検索一覧.md`（仕様書は作成済み。画面テストが未実施）
  - [ ] 18-7. マスタの新規登録と確認（MST-02 / MST-03） → `UT_11_マスタ新規登録.md`
  - [ ] 18-8. マスタ詳細（MST-04） → `UT_12_マスタ詳細.md`
  - [ ] 18-9. マスタ更新（MST-05 / MST-03） → `UT_13_マスタ更新.md`
  - [ ] 18-10. マスタ削除（MST-04） → `UT_14_マスタ削除.md`
...
| 項目 | 状態 |
| ------------ | ----------------------------------------------------------------------------------------------- |
| 作業ブランチ | `feat/master-management`。工程 17 まで push 済み、CI 成功 |
| 本番 DB | **マスタのマイグレーションは未適用**（`20260723125616_init` のみ）。ローカルは `20260812093057_add_master_export` まで適用済み。本番への適用は工程 18 |
| ローカル DB | Docker Compose の PostgreSQL 16 は**起動中**。マスタ分類 2 件・マスタ 35 件（ページング確認用）が入っている |
| ブラウザ検証 | ADMIN で CSV ダウンロード（MST-01 / MST-06、工程17分）、マスタ分類一覧（MST-06、工程18-1分。ADMIN/OPERATOR/VIEWER）は実機確認済み。工程 6〜11 分の残り・MST-02〜05・07〜10 のブラウザ操作は未実施のまま。**工程 18 の残りで検証する** |
| 直近の検証 | 2026-08-12、工程17実装後の状態で `lint` / `format:check` / `typecheck` / 13 ファイル 184 テストが成功。ローカル docker compose で CSV ダウンロード（依頼→生成→受け取り）を MST-01・MST-06 の両方で実機確認済み。2026-08-13、工程18-1でPlaywright（`--headed`）による画面操作テスト8ケースが全て成功 |
