...
| **マスタ分類の見直し** | **4 / 4** |
| **契約先・契約の管理画面整備** | **3 / 5（要件定義・設計・工程分解が完了、製造は工程1〜12へ具体化済み・未着手）** |
...
**契約先・契約の管理画面整備の設計書が完成した**（2026-08-19）。[`docs/specs/02_basic-design/party-contract/`](../specs/02_basic-design/party-contract/README.md)（13ファイル）に、マスタ管理画面と同じ画面構成に加え、確認画面・同時更新対策（楽観ロック）・登録者/更新者の表示・契約先を検索できるコンボボックスを新たに導入する方針をまとめた。詳細は [履歴](history/2026-08-w3.md#2026-08-19-契約先契約の管理画面整備の設計書を作成) を参照。

**次にやることは、[契約先・契約の管理画面整備](#契約先契約の管理画面整備)の4番・工程1（`Party`・`Contract`へ`createdBy`/`updatedBy`を追加するPrismaマイグレーションの作成・適用）から着手すること。**
...
- [ ] **4. 製造する**
  - [ ] 工程1. データベースに `createdBy`/`updatedBy` を追加するマイグレーションを作成・適用する（`Party`・`Contract`。[`01_データベース.md`](../specs/02_basic-design/party-contract/01_データベース.md)）
  - [ ] 工程2. 契約先の検索一覧画面（PTY-01）を実装する（検索条件・並び替え・ページング。埋め込みフォームと一覧の削除ボタンの撤去を含む。[`10_契約先検索一覧.md`](../specs/02_basic-design/party-contract/10_契約先検索一覧.md)）
  - [ ] 工程3. 契約先の新規登録・確認画面（PTY-02・PTY-03）を実装する（[`11_契約先新規登録.md`](../specs/02_basic-design/party-contract/11_契約先新規登録.md)）
  - [ ] 工程4. 契約先の詳細画面（PTY-04）を実装する（[`12_契約先詳細.md`](../specs/02_basic-design/party-contract/12_契約先詳細.md)）
  - [ ] 工程5. 契約先の更新画面（PTY-05）と楽観ロックを実装する（[`13_契約先更新.md`](../specs/02_basic-design/party-contract/13_契約先更新.md)）
  - [ ] 工程6. 契約先の削除確認ダイアログと削除処理（紐づく契約のチェックを含む）を実装する（[`14_契約先削除.md`](../specs/02_basic-design/party-contract/14_契約先削除.md)）
  - [ ] 工程7. 契約先コンボボックス（`party-combobox.tsx`）を実装する（契約の検索条件・登録・更新画面で使う共通部品。`pnpm dlx shadcn@latest add command popover` が必要）
  - [ ] 工程8. 契約の検索一覧画面（CTR-01）を実装する（検索条件・並び替え・ページング。埋め込みフォームと一覧の削除ボタンの撤去を含む。[`20_契約検索一覧.md`](../specs/02_basic-design/party-contract/20_契約検索一覧.md)）
  - [ ] 工程9. 契約の新規登録・確認画面（CTR-02・CTR-03）を実装する（[`21_契約新規登録.md`](../specs/02_basic-design/party-contract/21_契約新規登録.md)）
  - [ ] 工程10. 契約の詳細画面（CTR-04）を実装する（契約先詳細へのリンクを含む。[`22_契約詳細.md`](../specs/02_basic-design/party-contract/22_契約詳細.md)）
  - [ ] 工程11. 契約の更新画面（CTR-05）と楽観ロックを実装する（[`23_契約更新.md`](../specs/02_basic-design/party-contract/23_契約更新.md)）
  - [ ] 工程12. 契約の削除確認ダイアログと削除処理を実装する（[`24_契約削除.md`](../specs/02_basic-design/party-contract/24_契約削除.md)）
- [ ] **5. 単体テスト仕様書を作成し、画面操作テストを実施する**
