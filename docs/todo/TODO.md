...
| **不足ドキュメントの作成（棚卸し1＋要件定義・基本設計4＋単体テスト仕様書1＋テスト実施1）**             | **5 / 7**   |
| **公開用テンプレートの切り出し**                                            | **0 / 8**   |
...
**[不足ドキュメントの作成](#不足ドキュメントの作成)は2026-08-27に5番まで完了した**（対象範囲の確定、ログイン画面・トップ画面・マスタ機能・契約先契約管理の要件定義書、ログイン画面・トップ画面の基本設計書。→ [履歴](history/2026-08-w4.md#2026-08-27-マスタ機能契約先契約管理の要件定義書を作成不足ドキュメントの作成45番)）。**次のセッションでは、6番（ログイン画面・トップ画面の単体テスト仕様書の作成）に着手する。** 最初に `git status --porcelain` で未コミット差分を確認する。仕様書の作成には `create-unit-test-spec` スキルが使える。7番（6で作成した仕様書を使ったPlaywright画面操作テスト）は6番の完了後に着手する。不足ドキュメントの作成が完了したら、[公開用テンプレートの切り出し](#公開用テンプレートの切り出し)（2026-08-23 に方針を確定済み。→ [履歴](history/2026-08-w3.md#2026-08-23-公開用テンプレートの切り出し方針を確定)）へ進む。
