...
> **2026-07-28 変更**: 当初は Railway を本番ホスティング先としていたが、Railway の無料枠が実質廃止されている（$5 のトライアルクレジットは 30 日で失効し、以後の Free plan は $1/月クレジットのみ。継続運用には Hobby $5/月が必要）ことが判明したため、Google Cloud Run へ変更した。選定の経緯は [`docs/todo/TODO_履歴.md`](todo/TODO_履歴.md#2026-07-28-ホスティング先の選定) を参照。
