...
### 直前のセッションでやったこと（2026-08-02・PR 運用の開始、CI の順序バグ修正、Actions 更新）
...
- リポジトリ: `koekoebaborak27/multi-ai-agent-sample`（private）。**`main` = `a3173a0`**（初回コミット + PR #1 + PR #2 の計 3 コミット）。PR は 2 本ともマージ済み・ブランチ削除済み
...
コード・ドキュメントを変更する場合は、**`main` へ直接 push せず feature ブランチ → PR → CI green → squash マージ**の流れに従う（コマンドは上の「PR 運用の型が固まった」を参照）。Supabase / Cloud Run の設定作業そのものはリポジトリ外の操作なので、この流れの対象外。
