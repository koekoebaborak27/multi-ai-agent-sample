...
- [ ] **PR #1 をマージする**（CI green・conflict なし。マージ後に `git checkout main && git pull` でローカルを同期し、feature ブランチを削除する）
...
- [ ] **GitHub Actions のバージョンを更新する**（PR #1 のマージ後に別 PR で実施）。`actions/checkout@v4` / `actions/setup-node@v4` / `pnpm/action-setup@v4` は Node.js 20 をターゲットにしており非推奨。現在は GitHub 側が Node.js 24 で強制実行しているため CI は通るが、この救済措置が外れると**コード無変更のまま CI が壊れる**
  - 最新は **checkout v7 / setup-node v7 / pnpm/action-setup v6**（いずれも `action.yml` が `using: node24` であることを 2026-08-02 に確認済み）
  - `.nvmrc`（アプリ側の Node = 22）とは**無関係**。警告はアクション自身を動かす裏方の Node のことで、`.nvmrc` を変えても消えない
  - メジャー 3 世代跨ぎのため、各アクションのリリースノートで破壊的変更を確認してから上げること
...
### 直前のセッションでやったこと（2026-08-02・PR 運用の開始と CI の順序バグ修正）
...
PR: `koekoebaborak27/multi-ai-agent-sample#1`（**2026-08-02 00:10 時点で open・CI green・未マージ**）。
...
**3. GitHub Actions のバージョンが 3 世代遅れていることが判明**

CI は成功するが警告が 1 件出ている（詳細は「積み残し・確認事項」の該当項目）。
...
- リポジトリ: `koekoebaborak27/multi-ai-agent-sample`（private）。`main` は初回コミット `e498739` のまま
- **作業ブランチ `docs/agent-skills-update-todo` に 2 コミット（`0751372` / `3b83966`）。PR #1 は open・CI green・未マージ**
...
- CI（GitHub Actions）はグリーン。ステップ順序のバグ（Typecheck が Prisma generate より前）は修正済みで、修正自体は PR #1 に含まれているため **`main` にはまだ入っていない**
- CI に警告 1 件（アクションが Node.js 20 ターゲットで非推奨）。ビルドは通るが要対応 → 「積み残し・確認事項」参照
...
- 初回コミット以降の変更（Supabase 手順の追記・スキル追加）は**コミット済み。ただし PR #1 が未マージのため `main` には未反映**

### 次にやること

**まず PR #1 をマージし、続けて GitHub Actions の更新を別 PR で片付ける。** そのあと Supabase → Cloud Run の本筋へ戻る。

**1. PR #1 をマージしてローカルを同期する**
...
**2. GitHub Actions のバージョン更新（別 PR）**

```powershell
git checkout -b chore/bump-github-actions
```

[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) の 3 つの `uses:` を checkout v7 / setup-node v7 / pnpm/action-setup v6 へ上げる。メジャー 3 世代跨ぎのため、上げる前に各リリースノートで破壊的変更を確認すること。PR を作れば CI が検証してくれる。

**3. Supabase セクションの残り 3 項目**を上から順に進める。以降は Supabase → Cloud Run の依存順に並んでいるので、上から消化すればよい。
