...
**エージェントは常にトークン消費を意識する。** 必要な範囲だけを読み・調べ・出力する。具体的には以下を避ける。

- ドキュメント/コードの不要な全文読み込みや、範囲を絞らない広すぎる探索。ドキュメントはまずディレクトリの `README.md`（索引）か見出し検索で該当箇所を特定し、そのうえで必要な節だけを開く。20KB を超えるファイルを最初から全文読まない。読む前に `rg -n "^#{1,3} " <file>` で見出しを出せば、どこを読むべきか判断できる
- 必要性の薄いサブエージェント起動や、過剰な並列調査
- エージェントによるチャット上の回答・報告が冗長になること（不要な説明の繰り返しや過剰な要約など）
