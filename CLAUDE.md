...
- 権限設定は `.claude/settings.json`。ただし**ポリシーの正本は `docs/agent_permissions.md`**（Codex / Copilot と共有）。許可・禁止コマンドを変えるときはまずそちらを直し、`.claude/settings.json` / `.vscode/settings.json` / `.codex/config.toml` の 3 つへ反映してください。
